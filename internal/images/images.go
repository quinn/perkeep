...
// parseNumber strips any unit suffix and converts the leading
// number to float64.  Returns ok=false if it can’t.
func parseNumber(s string) (v float64, ok bool) {
...
// dimsFromSVG reads tokens until it sees the first <svg>
// start-element, then extracts width/height/viewBox.
func dimsFromSVG(r io.Reader) (w, h float64, err error) {
	...
	for {
		...
		if w, ok = parseNumber(widthAttr); ok {
			if h, ok = parseNumber(heightAttr); ok {
				return w, h, nil
			}
		}
		...
		if len(fields) == 4 {
			if w, ok = parseNumber(fields[2]); ok {
				if h, ok = parseNumber(fields[3]); ok {
					return w, h, nil
				}
			}
		}
...
func DecodeConfig(mimetype string, r io.Reader) (Config, error) {
	if mimetype == "image/svg+xml" {
		w, h, err := dimsFromSVG(r)
		if err != nil {
			return Config{}, err
		}
		return Config{
